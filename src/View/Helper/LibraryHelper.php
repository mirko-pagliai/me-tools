...
class LibraryHelper extends Helper {
	...
	protected function _datetimepicker($input, array $options = []) {
		...
		$options = optionDefaults('showTodayButton', TRUE, $options);
		
		//Shows the "Clear" button in the icon toolbar
		$options = optionDefaults('showClear', TRUE, $options);
		
		$options = optionDefaults('icons', [
			'time' => 'fa fa-clock-o',
			'date' => 'fa fa-calendar',
			'up' => 'fa fa-arrow-up',
		...
		$options = optionDefaults('locale', empty($locale) ? 'en-gb' : $locale, $options);
